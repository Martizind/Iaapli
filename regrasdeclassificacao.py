...
def classificar_por_regras(
    ...
    if any(
        ...
        return "Subscrições", "regra_subscricao"

    if any(
        token in description_ascii
        for token in ("MERCADONA", "ALDI", "LIDL", "CONTINENTE", "PINGO DOCE")
    ):
        return "Alimentação", "regra_supermercado"
    ...
    if any(
        ...
        return "Alimentação", "regra_restauracao"
